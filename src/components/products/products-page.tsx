...
// ★ v11.9.3: اصلاح Header - حذف تکرار و ساختار JSX صحیح
// ============================================================================
...
// ★ محاسبه ارزش کل موجودی یک کالا (موجودی × قیمت خرید)
function calculateTotalValue(currentStock: number, purchasePrice: number): number {
  return (currentStock || 0) * (purchasePrice || 0)
}
...
// ★ v11.9.1: کامپوننت نمایش ارزش موجودی (بر اساس قیمت خرید)
function StockValueBadge({ currentStock, purchasePrice }: { currentStock: number; purchasePrice: number }) {
  const totalValue = calculateTotalValue(currentStock, purchasePrice)
  if (totalValue <= 0) {
...
// ★ v11.9.1: کامپوننت نمایش ارزش موجودی در کارت موبایل (بر اساس قیمت خرید)
function StockValueCard({ currentStock, purchasePrice }: { currentStock: number; purchasePrice: number }) {
  const totalValue = calculateTotalValue(currentStock, purchasePrice)
...
export default function ProductsPage() {
    ...
    if (!trulyOnline) {
      ...
      try {
        ...
        const body = {
          ...
          initialStock: parsePersianNumber(addForm.initialStock),
    ...
    try {
      ...
      if (json.success) {
        logger.info('کالای جدید ثبت شد', {
          ...
          purchasePrice: json.data?.purchasePrice,
          salePrice: json.data?.salePrice,
          currentStock: json.data?.currentStock,
  ...
  const totalInventoryValue = useMemo(() => {
    return allProducts.reduce((sum, p) =>
      sum + calculateTotalValue(p.currentStock, p.purchasePrice), 0
    )
  }, [allProducts])
  ...
  return (
    <div className="space-y-3 sm:space-y-4" dir="rtl">
      {/* ═══════════════════════════════════════════════════════════════ */}
      {/* ★ v11.9.3: Header با آمار کامل انبار (ساختار JSX اصلاح شده)   */}
      {/* ═══════════════════════════════════════════════════════════════ */}
