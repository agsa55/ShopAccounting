...
export function WarehousesPage() {
  ...
  const handleOpenAdd = () => {
    if (!canAddWarehouse) {
      toast({
        title: 'محدودیت پلن',
        description: `در پلن فعلی فقط ${formatNumber(planInfo?.maxWarehouses)} انبار مجاز است.`,
        variant: 'destructive',
      })
      return
    }
      ...
      <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        ...
        <Button onClick={handleOpenAdd} className="gap-1.5 bg-emerald-600 hover:bg-emerald-700 w-full sm:w-auto h-9">
          <Plus className="w-4 h-4" />انبار جدید
              ...
              {/* ★★★ کارت هشدار محدودیت پلن (نسخه اجباری و بسیار فشرده) */}
      {planInfo && !canAddWarehouse && (
        <Card className="border-amber-200 bg-amber-50/50 shadow-none" style={{ minHeight: 'auto', padding: '0' }}>
          <CardContent className="flex items-center gap-2" style={{ padding: '6px 12px' }}>
            <Crown className="w-3.5 h-3.5 text-amber-600 shrink-0" />
            <p className="text-[10px] text-amber-700 leading-tight font-medium">
              سقف انبار پر شده است. برای افزودن انبار جدید، پلن خود را ارتقا دهید.
            </p>
          </CardContent>
        </Card>
      )}
          ...
          {warehouses.map((wh) => (
            <Card key={wh.id} className={wh.isDefault ? 'border-emerald-300 bg-emerald-50/30' : ''}>
             <CardContent className="p-3 sm:p-4">
                ...
                {/* ★ تمام اعداد با formatNumber فارسی شده‌اند */}
