...
const loadData = useCallback(async () => {
  ...
  try {
    ...
    if (err?.message?.includes('Failed to fetch') || err?.name === 'TypeError') {
      console.warn('[StockTransferPage] سوئیچ به آفلاین')
  ...
  if (!isOnline) {
    toast({ title: 'عدم دسترسی', description: 'ثبت انتقال نیاز به اتصال اینترنت برای به‌روزرسانی موجودی دارد', variant: 'destructive' })
    return
  }
